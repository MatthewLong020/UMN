...
struct image_info *parse_bcraw(FILE *fh) {
    ...
    /*
    Check for integer overflow with long long since long long is 64 bits
    instead of the 32 bits that long is and 16 bits for int
    */
    long long size_test = (long long)width * height * 3;

    num_bytes = 3 * width * height;

    if (size_test != num_bytes) {
        format_problem = "size mismatch, int overflow detected";
        return 0;
    }
...
void print_log_msg(struct image_info *info) {
    ...
    /*
        removed the fourth argument from the printf function since it enabled 
        format string injection and wasn't actually printed anyways in practice
    */
